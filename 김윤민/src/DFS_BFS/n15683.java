...
public class n15683 {
	...
	/* 1 : 한방향 , 2 : 180 2방향 , 3 : 90도 2방향 , 4 : 세방향, 5 : 네방향
	 * 
	 */
	static int N, M;
	...
	public static void main(String[] args) throws Exception {
		...
		for (int i = 0; i < N; i++) {
			...
			for (int j = 0; j < M; j++) {
				...
				// 벽이나 빈 곳이 아닐 때 = cctv인 경우 => 위치 + cctv종류 저장
	...
	public static void dfs(int cnt, int[][] map, ArrayList<Point> cctv) {
		...
		int cctvNum = cctv.get(cnt).cctvNum; //현재 cctv number
		int x = cctv.get(cnt).x;
		int y = cctv.get(cnt).y;
		int[][] tmp;
		if (cctvNum == 1) { //1방향
			tmp = copyMap(map); //map 복사
			checkLeft(tmp, x, y); 
			dfs(cnt + 1, tmp, cctv);
